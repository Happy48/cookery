...
public class NoteController {
     ...
     */
    @CrossOrigin
    @RequestMapping(value = "/api/getNoteUpdateInfo", method = RequestMethod.GET)
    public UpdateNoteVO getNoteUpdateInfo(@RequestParam("noteID") int noteID) {
        return noteService.getNoteUpdateInfo(noteID);
    }
     ...
     * @param tip
     * @param subtag
     * @return 更新笔记成功，code为noteID；子标签不在定义范围之内，code为1；没有该用户，code为2
     */
    @CrossOrigin
    @RequestMapping(value = "/api/updateNote", method = RequestMethod.POST)
    public HashMap<String, String> updateNote(@RequestParam("noteID") int noteID, @RequestParam("noteName") String noteName,
                                              @RequestParam("noteCover") String noteCover, @RequestParam("description") String description,
                                              @RequestParam("material") String material, @RequestParam("practice") String practice,
                                              @RequestParam("tip") String tip, @RequestParam("subtag") String subtag) {
        ...
        String[] subtagArr = subtag.split(",");
        List<String> subtagList = Arrays.asList(subtagArr);
        UpdateNoteVO updateNoteVO = new UpdateNoteVO(noteID, noteName, noteCover, description, material, practice, timestamp, tip, subtagList);
        int code = noteService.updateNote(updateNoteVO);
        resultMap.put("code", code + "");
